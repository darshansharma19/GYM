...
export default function StrongerEveryDay() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // ENTRANCE (0% - 30%)
      scrollTl
        .fromTo(rightImage, { x: '60vw', scale: 1.05, opacity: 0 }, { x: 0, scale: 1, opacity: 1, ease: 'none' }, 0)
  ...
  return (
    <section
      ref={sectionRef}
      className="section-pinned bg-gym-black z-40"
      style={{ height: '100vh' }}
    >
      {/* Background Image */}
      <div ref={bgRef} className="absolute inset-0">
        <img
          src="/images/scene_stronger_bg.jpg"
      ...
      >
        <img
          src="/images/scene_stronger_cut.jpg"
          alt="Athlete getting stronger"
          className="w-full h-full object-cover"
        />
      </div>
      ...
      >
        <h2 className="font-display font-black text-4xl sm:text-5xl lg:text-[clamp(34px,4.2vw,64px)] text-white uppercase leading-[1.1] tracking-[-0.02em] mb-6">
          Stronger
          <br />
          <span className="text-gym-orange">Every Day</span>
        </h2>
        <p className="text-gym-gray text-base lg:text-lg mb-8 leading-relaxed max-w-md">
          Progressive strength training, form coaching, and recovery guidance—so you keep improving without burning out. 
          Build a foundation that lasts.
        </p>
        <button
          onClick={() => {
            const element = document.querySelector('#trainers');
