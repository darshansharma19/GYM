...
export default function PushLimits() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      scrollTl
        ...
        .fromTo(text.querySelector('h2'), { x: '-40vw', opacity: 0 }, { x: 0, opacity: 1, ease: 'none' }, 0.05)
        .fromTo(text.querySelector('p'), { x: '-20vw', opacity: 0 }, { x: 0, opacity: 1, ease: 'none' }, 0.1)
        .fromTo(text.querySelector('button'), { x: '-20vw', opacity: 0 }, { x: 0, opacity: 1, ease: 'none' }, 0.15);

      // SETTLE (30% - 70%) - hold position

      // EXIT (70% - 100%)
      scrollTl
        .fromTo(rightImage, { x: 0, y: 0, opacity: 1 }, { x: '18vw', y: '-6vh', opacity: 0, ease: 'power2.in' }, 0.7)
  ...
  return (
    <section
      ref={sectionRef}
      className="section-pinned bg-gym-black z-30"
      style={{ height: '100vh' }}
    >
      {/* Background Image */}
      <div ref={bgRef} className="absolute inset-0">
        <img
          src="/images/scene_push_limits_bg.jpg"
          alt="Push limits"
      ...
      >
        <img
          src="/images/scene_push_limits_cut.jpg"
      ...
      >
        <h2 className="font-display font-black text-4xl sm:text-5xl lg:text-[clamp(34px,4.2vw,64px)] text-white uppercase leading-[1.1] tracking-[-0.02em] mb-6">
          Push Your
          <br />
          <span className="text-gym-orange">Limits</span>
        </h2>
        <p className="text-gym-gray text-base lg:text-lg mb-8 leading-relaxed max-w-md">
          Small-group coaching, expert feedback, and a program that evolves as you improve. 
          Break through plateaus and discover what you're truly capable of.
        </p>
        <button
          onClick={() => {
